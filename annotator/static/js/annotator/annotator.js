...
annotator = (function(self){
    ...
    self.get_article_coding_html = function(){
        ...
        return table.append($.map(widgets.get_html(schemafields, null), function(widget, i){
			...
			//field type id: 5 => combo box, 1 => text box
			//console.dir(self.state.article_coding.values[16].field.choices[0].label);
			//console.dir(self.state.article_coding);
			
			var varFieldHTML;
			
			if(schemafield.fieldtype == 5) {
				
				varFieldHTML = '<select style="width: 130px" id="'+schemafield.id+'" varfieldid="'+schemafield.id+'" class="cmbField">';
				for (i = 0; i < schemafield.choices.length; i++) {
					
					if (value !== undefined && value.intval !== undefined){
						if(value.intval == schemafield.choices[i].intval) {
							varFieldHTML += '<option value="'+schemafield.choices[i].intval+'" selected>'+schemafield.choices[i].label+'</option>'
						} else {
							varFieldHTML += '<option value="'+schemafield.choices[i].intval+'">'+schemafield.choices[i].label+'</option>'
						}
					} else {
						varFieldHTML += '<option value="'+schemafield.choices[i].intval+'">'+schemafield.choices[i].label+'</option>'
					}
				...
				if (value !== undefined && value.strval !== undefined)
					varFieldHTML = '<input type="text" id="'+schemafield.id+'" varfieldid="'+schemafield.id+'" value="'+value.strval+'"  class="txtField" />';
				else {
					varFieldHTML = '<input type="text" id="'+schemafield.id+'" varfieldid="'+schemafield.id+'"  class="txtField" />';
				}
			}
	...
	//console.log("test1");
        ...
        self.article_coding_container.find("tr")
            ...
            .bind("click", function() {
				...
				$("#btnHighlightKeywords").text("Highlight Keywords");
				$("#btnHighlightKeywords").attr("disabled", false);
				$("#btnHighlightKeywords").attr("title", "Highlight the keywords of respective question in the article.");
    ...
    self.get_article = function(coded_article_id){
		...
		//setTimeout(function() { $("#loadingHighlight").fadeOut(); }, 60000);
	...
	console.log(sentences);
	    ...
	    j++;
            ...
            if (s.get_unit() == '1.1') {
                ...
                //_html.append($('<div />').addClass('annotator-sentencenr').text('(' + s.get_unit() + ') '));
				_html.append($('<div />').addClass('annotator-sentencenr').attr('style', 'float: left; width: 35px;').text('(' + s.get_unit() + ') '));
                //_html.append(document.createTextNode(s.sentence));
                _html.append('<div style="margin-left: 35px;">' + s.sentence + '</div>');
            }
            prev_parnr = s.parnr;
            html.append(_html);
        });
    ...
    self.datatables_row_clicked = function(row){
		...
		//Code by Aemal Sayer
		//Store the article coded ID in session storage, it will be used in ajax calls for saving coded values.
		sessionStorage.setItem('varCodedArticleID', coded_article_id);
    };
